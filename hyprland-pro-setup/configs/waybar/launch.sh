...
if [ ! -f $HOME/.config/ml4w/settings/waybar-disabled ]; then
    ...
    HYPRLAND_INSTANCE_SIGNATURE="$HYPRLAND_SIGNATURE" waybar -c ~/.config/waybar/themes${arrThemes[0]}/$config_file -s ~/.config/waybar/themes${arrThemes[1]}/$style_file > /tmp/waybar_err.log 2>&1 &
    # env GTK_DEBUG=interactive waybar -c ~/.config/waybar/themes${arrThemes[0]}/$config_file -s ~/.config/waybar/themes${arrThemes[1]}/$style_file > /tmp/waybar_err.log 2>&1 &
else
